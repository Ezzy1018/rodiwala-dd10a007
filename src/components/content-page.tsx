...
export function MaterialTile({ material }: { material: Material }) {
  return <article className="catalogue-tile"><img src={material.image} alt={`Illustrative ${material.name}`} width={800} height={680} loading="lazy"/><div className="catalogue-shade"/><div className="catalogue-copy"><p>Core material</p><h3>{material.shortName}</h3><span>{material.description}</span></div><div className="catalogue-actions"><a href={quoteHref(material.name)} target="_blank" rel="noreferrer" className="catalogue-action" aria-label={`Get a quote for ${material.name}`}>Get a quote <ArrowRight size={17}/></a><Link to={material.href} className="catalogue-link">Details</Link></div></article>;
}
