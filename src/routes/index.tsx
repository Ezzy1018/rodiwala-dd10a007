...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Rodiwala supplies 10 mm and 20 mm aggregate (rodi), stone dust, M-sand and ready-mix concrete to sites across India. Send your requirement for delivered pricing." },
    { property: "og:title", content: "Construction Material Supplier in India | Rodiwala" },
    { property: "og:description", content: "Share your material, quantity, site location and date from anywhere in India. Rodiwala confirms availability and delivered pricing." },
  ...
  return (
    <main>
      <section className="home-hero">
        ...
        <div className="home-hero-content"><p className="eyebrow text-accent"><span className="status-dot"/>Construction material supply across India</p><h1>Construction materials for a stronger tomorrow.</h1><p>10 mm & 20 mm aggregate (rodi), stone dust, M-sand and ready-mix concrete—quoted for your quantity, site and required date, wherever in India you are building.</p><div className="mt-8"><QuoteButtons context="Homepage hero"/></div></div>
        <div className="home-hero-facts"><span><strong>India</strong>-wide enquiries</span><span><strong>5</strong> core materials</span><span><strong>1</strong> direct conversation</span><span><strong>0</strong> checkout guesswork</span></div>
      </section>
      ...
      <section className="content-band"><div className="mx-auto grid max-w-7xl items-center gap-10 px-5 md:grid-cols-[.85fr_1.15fr] lg:px-8"><div><p className="eyebrow">Delivery coverage</p><h2 className="section-title">Your location shapes the quote.</h2><p className="mt-4 max-w-lg text-lg text-muted-foreground">Rodiwala takes requirements from sites across India. Feasibility depends on the nearest source, load, route and site access—so share the exact city, locality or pin code and the route gets checked for you.</p><ul className="mt-6 space-y-3 text-sm text-foreground">{["Freight basis stated clearly","Vehicle access checked","Timing confirmed before dispatch"].map((x)=><li className="flex gap-3" key={x}><Check className="shrink-0 text-primary" size={19}/>{x}</li>)}</ul><Link to="/delivery-areas" className="btn-outline mt-7">Check delivery factors <ArrowRight size={17}/></Link></div><div className="image-frame"><img src={deliveryImage} alt="Tipper truck delivering construction aggregate at a site" width={1200} height={800} loading="lazy"/><img src={logoMark} alt="" className="image-frame-mark"/></div></div></section>
